...
import com.soothe.sapApplication.Global_Classes.GlobalMethods.numberToK
...
class InventoryTransferItemAdapter(
    ...
    var list: ArrayList<InventoryRequestModel.StockTransferLines>,
    private val networkConnection: NetworkConnection,
    private val materialProgressDialog: MaterialProgressDialog,
    private val callback: InventoryTransferLinesActivity,
...
) : RecyclerView.Adapter<InventoryTransferItemAdapter.ViewHolder>(),
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with(holder) {
            with(myObject) {
                ...
                binding.tvTotalBaseQuantity.text = ":   " + this?.Quantity
                binding.tvOpenQty.text = ":   " + this?.ItemCode
                /* var qty = this.Quantity.split(".")
                 var boxQty =qty[0].toInt()/U_IQTY.toInt();*/

                var qtyBox = 0;
                if (numberToK(this!!.U_IQTY)!!.toInt() > 0)
                    qtyBox = numberToK(this!!.RemainingOpenQuantity)!!.toInt() / numberToK(this.U_IQTY)!!.toInt()

                var doBoxQuantity = 0;
                if (numberToK(this!!.U_IQTY)!!.toInt() > 0)
                    doBoxQuantity = numberToK(this!!.Quantity)!!.toInt() / numberToK(this.U_IQTY)!!.toInt()
...
//                binding.tvNavicode.text = this.NavisionCode?.let { date -> " : " + date } ?: " :"//todo cmmented

                binding.tvNavicode.text =  " :   " + this.NavisionCode
                            ...
                            if (keyEvent.action == KeyEvent.ACTION_UP && keyCode == KeyEvent.KEYCODE_ENTER) {
                                ...
                                try {
                                    ...
                                    if (x.contains("/")) {
                                        ...
                                        scanNavCodeItem(x.split("/")[0], position, "")

                                    } else {
                                        //todo scan call api here...
                                        scanNavCodeItem(text.split("/")[0], position, "")
               ...
               /* Handler(Looper.getMainLooper()).postDelayed({
                    ...
                    if (imm != null && laserCode != null) {
                        imm.hideSoftInputFromWindow(laserCode.windowToken, 0)
                    }
                }, 200)*/
...
//                        remainingOpenQuantity = this.RemainingQuantity
                        ...
                        if (sessionManagement.getScannerType(context) == null) {
                            ...
                            val intent = Intent(context, QRScannerActivity::class.java)
                            Log.e("Manual Scan","+"+list.get(adapterPosition).ItemCode);
                ...
                save.setOnClickListener {
                    ...
                    if (AppConstants.IS_SCAN == false ){
                        ...
                        save.isEnabled = false
                        save.isCheckable = false
                        callback.onApiResponseStock(hashMap, list)
                    }
//                    callback.onApiResponseStock(hashMap, list)//todo comment for now
                }
    ...
    fun checkDuplicate(
        scanedBatchedItemsList_gl: ArrayList<ScanedOrderBatchedItems.Value>,
        batchCode: String
    ): Boolean {
                        ...
                        override fun onResponse(
                            ...
                            try {
                                ...
                                if (response.isSuccessful) {
                                    ...
                                    } else if (list[itemPo].totalPktQty.toDouble() >= list[itemPo].RemainingOpenQuantity.toDouble()) {
                                        GlobalMethods.showError(
                                            context,
                                            "Scanning completed for this Item"
                                        )
                                    } else {
                                        if (list.size > 0 && !responseModel.value.isNullOrEmpty()) {


                                        /*     if(list[itemPo].totalPktQty.toDouble()>=list[itemPo].RemainingOpenQuantity.toDouble()) {
                                            ...
                                            list[itemPo].totalPktQty =
                                                responseModel.value[0].U_PACK_QTY.toInt() * scanCount;
                                            list[itemPo].totalPktQty =
                                                responseModel.value[0].U_PACK_QTY.toInt() * scanCount;
                                            list[itemPo].NavisionCode = text
    ...
    // Filtering Data from List on ItemCode
...
//                    tvOpenQty.text = "  : "+GlobalMethods.sumBatchQuantity(position, quantityHashMap.get("Item" + position)!!).toString()
                                ...
                                if (response.isSuccessful) {
                                    ...
                                    if (responseModel.value.size > 0 && !responseModel.value.isNullOrEmpty()) {
                                        ...
                                        if (!itemList_gl.isNullOrEmpty()) {
                                            ...
                                            var totalGrossWeight = GlobalMethods.changeDecimal(
                                            ...
                                            if (quantityHashMap.get("Item" + position)!!.size > 0) {
                                                if (!quantityHashMap.get("Item" + position)!!
                                                    ...
                                                    batchItemsAdapter?.setOnDeleteItemClickListener(
                                                        this@InventoryTransferItemAdapter
                                                    )
                                                ...
                                                GlobalMethods.showError(
                                                    context,
                                                    "No Quantity Found of this Production Order."
                                                )
                                            }
    ...
    private fun setScanDataOnItem(
        arrayList: ArrayList<InventoryRequestModel.StockTransferLines>,
        itemCode: String
    ): Int {
    ...
    open fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            data?.getStringExtra("batch_code").toString()
    ...
    private fun setSqlServer() {
        Log.e("ConStatus", "Call setSqlServer() in InventoryTransferItemAdapter")
        try {
    ...
    private fun scannerPopupDialog(
        text: String,
        rvBatchItems: RecyclerView,
        context: Context,
        position: Int,
        ...
        goBtn?.setOnClickListener {
            if (checkGender.equals("L")) {
                ...
                val intent = Intent(context, QRScannerActivity::class.java)
                pos = position
                (context as ProductionOrderLinesActivity).startActivityForResult(
                    intent,
                    REQUEST_CODE
                )
            }
    ...
    fun laserScannerPopupDialog(
        ...
        ed_batch_code_scan.setOnTouchListener(OnTouchListener
        { view1: View, motionEvent: MotionEvent? ->
            ed_batch_code_scan.setText("")
